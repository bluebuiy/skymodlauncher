...
std::string QuickDigest5Hash::to_string() const
{
    std::stringstream ss;
    for(int i = 0; i < 16; ++i)
        ss << std::hex
...
bool QuickDigest5Hash::from_string(std::string const & hashStr)
{
    this->hash.fill(0);
    if (hashStr.size() != 32)
    {
    ...
    auto ctoi = [](char c, bool & err) -> uint8_t
    {
        if (c >= '0' && c <= '9')
        {
            return c - '0';
    ...
    {
        char c1 = hashStr[i * 2];
        char c2 = hashStr[i * 2 + 1];
        uint8_t value;
        value = ctoi(c1, err) | (ctoi(c2, err) << 4);
        hash[i] = value;
    }
...
void QuickDigest5::step(const std::array<uint32_t, 16>& inputVec) {
...
QuickDigest5Hash QuickDigest5::digestString(const std::string& input) {
    QuickDigest5 QuickDigest5;
    QuickDigest5.update(
    ...
    return QuickDigest5Hash{QuickDigest5.digest};
}

QuickDigest5Hash QuickDigest5::digestFile(std::istream& file) {
